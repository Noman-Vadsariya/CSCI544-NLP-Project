...
def combine_and_shuffle_parquet_files(dataset_folders, output_train_path, output_test_path):
    """
        ...
        for root, _, files in os.walk(folder):
            for file in files:
                if file.endswith(".parquet"):
                    ...
                    if "train" in root.lower():  # Check if the file is in a train directory
                        print(f"Adding train file: {file_path} with {num_samples} samples")
                        train_datasets.append(dataset)
                    elif "test" in root.lower():  # Check if the file is in a test directory
                        print(f"Adding test file: {file_path} with {num_samples} samples")
...
if __name__ == "__main__":
    ...
    output_train_file = "./raw_datasets/combined_gold_compact/train/ds.parquet"
    output_test_file = "./raw_datasets/combined_gold_compact/test/ds.parquet"

    combine_and_shuffle_parquet_files(dataset_folders, output_train_file, output_test_file)