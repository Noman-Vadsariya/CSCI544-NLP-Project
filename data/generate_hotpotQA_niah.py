...
def extract_answer_from_needle(needle_text: str) -> str:
    match = re.search(r"\bis\s+(.+?)\.\s*$", needle_text.strip())
    if match:
        return match.group(1).strip()
    raise ValueError(f"Could not extract answer from needle text: {needle_text!r}")


def load_needles(needles_path: str) -> list[NeedleEntry]:
    with open(needles_path, "r", encoding="utf-8") as f:
        payload = json.load(f)

    raw_needles = payload["data"] if type(payload) is dict and "data" in payload else payload

    needles = []
    for row in raw_needles:
        needle_text = row["needle"].strip()
        question = row["question"].strip()
        answer = extract_answer_from_needle(needle_text)
        needles.append(NeedleEntry(needle=needle_text, question=question, answer=answer))

    return needles


def make_hotpotqa_distractor_pool(
    contexts: list[str],
    tokenizer,
    chunk_size: int,
    overlap: int,
    max_chunks_per_example: int = 4,
) -> list[str]:
    pool = []
    for ctx in contexts:
...
def build_examples_from_hotpotqa(
    ...
    candidate_indices = [i for i, q in enumerate(questions) if q and q.strip()]
    rng.shuffle(candidate_indices)

    shuffled_needles = needles[:]
    rng.shuffle(shuffled_needles)

    n_samples = min(n_samples, len(candidate_indices), len(shuffled_needles))

    samples = []
    for used, (ex_idx, needle_entry) in enumerate(
        zip(candidate_indices[:n_samples], shuffled_needles[:n_samples])
    ):
        source_hotpot_question = questions[ex_idx]
        question = needle_entry.question
        answer = needle_entry.answer
        needle_text = needle_entry.needle
        depth_bin = used % 10

        if use_fixed_noise_only:
            blocks = [NOISE_BLOCK for _ in range(n_distractors)]
        else:
            blocks = sample_hotpotqa_blocks(
                rng,
                context_pool,
                answer,
        ...
        samples.append(
            NIAHSample(
                ...
                source_hotpot_question=source_hotpot_question,
                question=question,
                answer=answer,
                context=context,
                prompts=[question],
...
def main() -> None:
    ...
    parser.add_argument("--needles-path", type=str, default="data/raw_datasets/hotpotQA_niah/needles.json")
    ...
    parser.add_argument(
        ...
        type=int,
        default=40,
        help="Heuristic tokens per block, matching the repo's TOKENS_PER_BLOCK.",
    )
    ...
    parser.add_argument(
        "--fixed-noise-only",
        action="store_true",
        help="Use the paper's exact fixed noise block instead of HotpotQA chunks.",
    )
    ...
    questions, contexts = load_hotpotqa_examples(args.data_path)
    needles = load_needles(args.needles_path)
    ...
    meta = {
        ...
        "needles_path": args.needles_path,
